...
//                .addImportantVertexes(innerHexes.stream().map(h->ImportantVertex.of(h, ONE, String.valueOf(vIndex.getAndIncrement()))).collect(toList()))
                .addImportantVertexes(Stream.of(ONE).map(v -> indexOnVertexes.apply(innerHexesHor, v)).map(Collection::stream).flatMap(s -> s).collect(toList()))
                .addImportantVertexes(Stream.of(TWO, FIVE).map(v -> indexOnVertexes.apply(innerHexesVer, v)).map(Collection::stream).flatMap(s -> s).collect(toList()))
